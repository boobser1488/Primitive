...
/// Fog when the camera is submerged -- murky green-blue, and much closer in.
pub const UNDERWATER_FOG: Vec3 = Vec3::new(0.10, 0.28, 0.38);
...
impl Sky {
    ...
    /// Fog is tinted toward the sky so distant terrain dissolves into the
    /// horizon instead of into a grey band. That's the whole trick to
    /// making a limited render distance not look like a limited render
    /// distance.
    pub fn fog_color(&self, underwater: bool) -> Vec3 {
        if underwater {
            UNDERWATER_FOG
        } else {
            let sky = self.sky_color();
            sky.lerp(Vec3::splat(sky.length() / 3f32.sqrt()), 0.15)
        }
    }
