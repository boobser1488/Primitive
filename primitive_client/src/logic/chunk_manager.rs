...
pub struct ChunkManager {
    ...
    /// without copying it.
    ///
    /// A chunk is 32 KB, and lighting one -- the expensive half of
    /// bringing it into the world -- runs off the main thread on a copy
    /// of its blocks. At the rate terrain streams that copy was
    /// megabytes a second of pure memcpy. Sharing it costs a reference
    /// count instead, and the one case where sharing is not free (an
    /// edit arriving while a worker still holds the chunk) is handled by
    /// `Arc::make_mut`, which copies exactly then and only then.
    loaded: HashMap<ChunkPos, std::sync::Arc<Chunk>>,
...
impl ChunkManager {
    ...
    pub fn insert(&mut self, chunk: Chunk) -> std::sync::Arc<Chunk> {
