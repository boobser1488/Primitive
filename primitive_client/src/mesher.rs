...
use crate::mesh::{build_mesh, MeshBuffers, Neighbourhood};
...
enum Job {
    ...
    /// Light a chunk in isolation. Pure computation over a copy of the
    /// chunk's blocks -- the expensive half of chunk integration, and
    /// the half that doesn't touch the shared light map.
    Light {
        pos: ChunkPos,
        blocks: Vec<BlockId>,
    },
...
impl Mesher {
            ...
            std::thread::Builder::new()
                ...
                .spawn(move || loop {
                    ...
                    let result = match job {
                        ...
                        Job::Light { pos, blocks } => Finished::Light {
                            pos,
                            data: compute_isolated(&blocks),
                        },
                    };
    ...
    pub fn submit_lighting(&mut self, pos: ChunkPos, blocks: Vec<BlockId>) {
        if self.jobs.send(Job::Light { pos, blocks }).is_ok() {
...
mod tests {
    ...
    fn test_layers() -> FaceLayers {
        crate::texture::FaceLayers::empty_for_test()
    }
    ...
    fn lighting_runs_on_the_workers_too() {
        ...
        mesher.submit_lighting(ChunkPos::new(2, 3), blocks);
