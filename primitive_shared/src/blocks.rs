...
pub const BLOCKS: &[BlockDef] = &[
    ...
    // **A saddle, one to a slot and seven kilos**: a wooden tree and a
    // leather seat is a real weight in a pack, so carrying a spare to the
    // hills is a choice. See `types::BLOCK_SADDLE`.
    BlockDef {
        id: BLOCK_SADDLE,
        name: "saddle",
        shape: Shape::Item,
    ...
    // Saddlebags, empty: two panniers of leather, light until they are on a
    // horse and full (`horse::load_factor`).
    BlockDef {
        id: BLOCK_SADDLEBAGS,
        name: "saddlebags",
        shape: Shape::Item,
    ...
    // **The saws** (`types::BLOCK_COPPER_SAW`). Their lives are the
    // hammers' and not the axes': a saw is worn by a log a stroke at a time
    // (`crafting::used_tool`), never by a swing at a block, and a log is a
    // bigger bite than a nail. The edge is what goes first
    // (`tools::edge_swings`); the whetstone brings it back.
    BlockDef {
        id: BLOCK_COPPER_SAW,
        name: "copper_saw",
        shape: Shape::Item,
    ...
    // **The sawhorse and the honing stone**, the two stations of the edge
    // (`minigame::Game::Saw`, `Game::Whet`). Faced like the workshops -- a
    // joiner stands at the end of the beam, a grinder in front of the slab --
    // and propped, for the wheel's reason: a station standing on nothing is
    // a station nobody meant.
    BlockDef {
        id: BLOCK_SAWHORSE,
        name: "sawhorse",
        shape: Shape::Cube,
    ...
    // **The horse's carcass**, on the zebra's row and a little bigger: half
    // a cell of dead animal, and the heaviest thing a hunt leaves on the
    // plain. See `animals::Species::Horse`.
    BlockDef {
        id: BLOCK_CARCASS_HORSE,
        name: "carcass_horse",
        shape: Shape::Cube,
        thickness: 4,
        matter: Matter::Solid,
